...
%% Dyamics
f = @(t,X) [X(2) ; X(1) - 1/3.*X(1).^3 - X(2) ] %#ok<NOPTS>
...
for a = a0:as:af
  ...
  saveas(fig, sprintf('fig/exemplo1_quadrado_%d.eps', index), 'epsc');
  print(fig, sprintf('fig/exemplo1_quadrado_%d.png', index), '-dpng', '-r300');
  index = index + 1;
end
